...
var contractWithMultipleInputs = ContractBuilder.newContract()
    ...
    .build();
----
<1> For file contract inputs, the file to use must be added to the classpath, and the corresponding path has to be used in the contract builder. For example, put your files in `src/test/resources/documents` and retrieve the resource URL like this: `MyTest.class.getResource("/documents/<FILE NAME>")`.
