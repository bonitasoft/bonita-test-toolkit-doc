...
----
<1> `BonitaTests` is an annotation that registers a JUnit 5 extension. It's a convenient way to set up a Bonita test class. This extension allows injecting a `BonitaTestToolkit` correctly instantiated in the test methods.
...
<3> The toolkit offers the possibility to retrieve an existing user on the targeted Bonta platform. This user is used to execute processes during the test scenario.
