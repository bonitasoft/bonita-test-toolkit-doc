...
Business data can be retrieved from a process instance at any time during a test, so their values can be validated as much as necessary to ensure that the process behaves correctly. +
...
A business data is defined at process level, it represents a business object from the xref:data:define-and-deploy-the-bdm.adoc[Business Data Model]. +
...
The second one is a bit more expensive since it requires a Java model matching the Business Data Model, but then it eases the use of the business data by providing a direct access to the fields.
